...
/datum/unit_test/mob_overlay_icons/Run()
	...
	for(var/obj/item/item_path as anything in types_to_search())
		...
		if(isnull(icon_state) || !istext(icon_state)) {
			TEST_NOTICE(src, "[item_path] skipped due to missing or invalid icon state.")
		...
		if(mob_overlay_icon) {
			...
			if(!fexists(mob_overlay_icon)) {
				TEST_NOTICE(src, "[item_path] skipped due to missing mob_overlay_icon file: '[mob_overlay_icon]'.")
				continue
			}
