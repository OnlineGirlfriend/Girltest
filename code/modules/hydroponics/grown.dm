	...
	switch(O.tool_behaviour)
		if(TOOL_SCREWDRIVER)
			...
			if(do_after(user, 28, target = src))
				to_chat(user, "<span class='notice'>You dig into the [src] to collect its seeds! It's all gross and unusuable now, ew!</span>")
		...
		if(TOOL_WIRECUTTER)
			...
			if(do_after(user, 28, target = src))
				to_chat(user, "<span class='notice'>You nip into the [src] to collect its seeds! It's all gross and unusuable now, ew!</span>")
				seedify(src, 1, TRUE, TRUE, src, user)
		if(TOOL_CROWBAR)
			...
			if(do_after(user, 20, target = src))
				to_chat(user, "<span class='notice'>You split apart the [src]! Sadly you put too much force and its remains are unusable, but hey, you got your seeds!</span>")
				seedify(src, 1, TRUE, TRUE, src, user)
		if(TOOL_WRENCH)
