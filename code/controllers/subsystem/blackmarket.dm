		...
		switch(purchase.method)
			...
			if(SHIPPING_METHOD_DEAD_DROP)
				...
				if(!landing_turf)
					...
					to_chat(recursive_loc_check(purchase.uplink.loc, /mob), "<span class='notice'>[purchase.uplink] flashes a message that the pod was unable to reach its designated landing spot and has landed somewhere in the local area instead.</span>")
